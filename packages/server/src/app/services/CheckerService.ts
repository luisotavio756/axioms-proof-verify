    ...
    public execute({
        ...
        switch (type) {
            ...
            case 'modus_ponens':
                if (!formulasToMP) {
                    throw new AppError(
                        'The formulas to compare are requireds!',
                        400,
                    );
                } else {
                    ...
                    if (!baseFormula || !toCompareFormula) {
                        throw new AppError(
                            'The indicate formulas not exists!',
                            404,
                        );
                    }
                    ...
                    if (
                        formulasToMP[0] >
                            this.formulasRepository.findAll().length ||
                        formulasToMP[1] >
                            this.formulasRepository.findAll().length
                    ) {
                        throw new AppError(
                            'The indicate formulas are invalids!',
                            400,
                        );
                    }

                    isTruthy = utils.verifyModusPonens(
                        baseFormula,
                        toCompareFormula,
                    );
                }
